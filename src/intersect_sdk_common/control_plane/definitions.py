"""Types and data structures for control plane logic, used by the SDK and core services."""
...
1. The PAYLOAD of the message, in raw bytes. The payload will be either the domain science properties (event/userspace messages), lifecycle information (lifecycle messages), or the pointer to the data (if the data handler is not MESSAGE - parse headers to get this).
