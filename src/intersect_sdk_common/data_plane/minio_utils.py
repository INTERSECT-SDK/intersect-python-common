"""MINIO functionality used by the DataPlaneManager."""
...
def create_minio_store(config: DataStoreConfig) -> Minio:
    ...
    config : DataStoreConfig
        The data store configuration containing host, username, and password.

    Returns:
    --------
    Minio
        A validated MINIO client instance.

    Raises:
    -------
    IntersectError
        If the host cannot be parsed as a valid hostname or if credentials are invalid.
    """
    config_uri = parse_url(config.host)
    if not config_uri.host:
        msg = f'Minio configuration host {config.host} cannot be parsed as a valid hostname.'
        raise IntersectError(msg)
    ...
    try:
        ...
        msg = f'Invalid credentials for Minio instance: {config}'
        raise IntersectError(msg) from e
    return client
